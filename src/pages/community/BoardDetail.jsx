...
export default function BoardDetail() {
  ...
  async function loadPost() {
    ...
    try {
      ...
      if (data) {
        // Increment views via RPC or direct update
        try {
          await supabase.rpc('increment_autowork_post_views', { post_uuid: id });
        } catch {
          await supabase.from('autowork_board_posts').update({ views: (data.views || 0) + 1 }).eq('id', id);
        }
  ...
  async function handleComment(e) {
    ...
    await supabase.from('autowork_board_comments').insert({
      post_id: id,
      user_id: user.id,
  ...
  async function handleDeletePost() {
    const { error } = await supabase.from('autowork_board_posts').delete().eq('id', id);
  ...
  async function handleDeleteComment(commentId) {
    const { error } = await supabase.from('autowork_board_comments').delete().eq('id', commentId);
    if (error) { toast.error(error.message); return; }
